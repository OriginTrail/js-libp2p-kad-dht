...
export class TestDHT {
    ...
    async spawn(options = {}, autoStart = true) {
        ...
        const dht = new DualKadDHT(new KadDHT({
            protocolPrefix: '/ipfs',
            lan: false,
            ...opts
        }), new KadDHT({
    ...
    async connect(dhtA, dhtB) {
        ...
        async function checkConnected(a, b) {
            ...
            return await Promise.all(routingTableChecks
                .map(async (check) => await pRetry(check, { retries: 50 })));
        }
    }
    async teardown() {
        await Promise.all(Array.from(this.peers.entries()).map(async ([_, { dht }]) => await dht.stop()));
        this.peers.clear();
    }
}
