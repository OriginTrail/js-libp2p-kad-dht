...
describe('KadDHT', () => {
    ...
    describe('peer routing', () => {
        ...
        it('find peer query', async function () {
            ...
            // Create 101 nodes
            const nDHTs = 100;
            const dhts = await Promise.all(new Array(nDHTs).fill(0).map(async () => await tdht.spawn()));
            const dhtsById = new Map(dhts.map((d) => [d.components.getPeerId(), d]));
            ...
            source => map(source, (event) => event.peer.id), async (source) => await all(source));
