...
describe('Routing Table', () => {
    ...
    it('removes tags from kad-close peers when closer peers are found', async () => {
        async function getTaggedPeers() {
            return new PeerSet(await pipe(await components.getPeerStore().all(), async function* (source) {
        ...
        const sortedPeerList = await sortClosestPeers(await Promise.all(new Array(KBUCKET_SIZE + 1).fill(0).map(async () => await createEd25519PeerId())), localNodeId);
